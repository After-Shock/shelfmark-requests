...
# Backported from upstream calibrain/shelfmark #1105: Debian's chromium
# 150.0.7871.46-1~deb13u1 security update (trixie-security, 2026-07-05) no
# longer opens the DevTools remote-debugging TCP port, so the SeleniumBase
# Pure-CDP driver cannot connect and every internal bypass fails with
# "Pure CDP browser startup failed" — all CF-gated downloads break.
# Install the last working version from snapshot.debian.org until the
# bypasser can talk to Chromium >= 150 or seleniumbase ships a fix.
