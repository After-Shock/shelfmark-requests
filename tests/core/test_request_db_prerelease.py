...
def test_create_update_list_and_count_no_sources_requested(db):
    ...
    assert len(listed) == 1
    assert listed[0]["id"] == req["id"]
    assert listed[0]["status"] == "no_sources_requested"

    assert db.count_requests(user_id=1, status="no_sources_requested") == 1
...
def test_initialize_migrates_existing_v7_schema_to_allow_no_sources_requested(tmp_path):
