...
import { ContentType, RequestCounts } from '../types';
  ...
  return (
    ...
    >
      <div className="max-w-full mx-auto px-4 sm:px-6 lg:px-8 py-4">
        ...
        {showSearch && (
          <div className="flex flex-col lg:flex-row lg:justify-between lg:items-center gap-3 animate-pop-up">
            ...
            <div className="flex items-center gap-4 lg:order-1 flex-1">
              {/* Theme toggle button - upper left */}
              {onThemeToggle && (
                <button
                  onClick={onThemeToggle}
                  className="flex items-center justify-center w-10 h-10 rounded-full hover-action transition-colors flex-shrink-0"
        ...
        {/* When search is NOT active: show theme toggle on left, icon buttons on right */}
        {!showSearch && (
          <div className="flex items-center justify-between min-h-[48px]">
            ...
            {onThemeToggle && (
              <button
                onClick={onThemeToggle}
                className="flex items-center justify-center w-10 h-10 rounded-full hover-action transition-colors"
                aria-label={`Switch to ${theme === 'dark' ? 'light' : 'dark'} mode`}
                title={`Switch to ${theme === 'dark' ? 'light' : 'dark'} mode`}
              >
                {theme === 'dark' ? (
